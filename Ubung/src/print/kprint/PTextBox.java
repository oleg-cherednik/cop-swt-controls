...
/**
 * A printable text label that can occupy more than one page. If you are shure that the box will not be bigger than one
 * page, you can use PLittleTextBox to make the layout process quicker.
 * 
...
class PTextBox extends PBox
{
	 ...
	 * @param parent
	 * @param style
	 * @param hWeight Specify -1 for a non-wrapping text box (If the text has newlines it will be a multi-line textbox).
	 *            Spezify a number between 0 and 1 for a multiline textbox that consumes the given fraction of the
	 *            available document width.
	 * @param minWidth This allows you to specify a minimum width for the text. The text box will consume some space
	 *            depending to hWeight or its text if hWeight is -1, but at least the given amount of centimeters. For a
	 *            box with a fixed width for example set hWeigth = 0 and specify a non-zero minWidth.
	 */
		...
		if(layoutIsOnPage(page))
		{
			...
			for(int i = 0; i < part.numOfLines; i++)
			{
				...
				if(textStyle.textAlign == PTextStyle.ALIGN_CENTER)
				{
					...
					alignPixel =
					                (getWidth() - pixelX(textStyle.getMarginLeft())
				...
				gc.drawText((String)textLines.get(part.startLine + i), part.origin.x + alignPixel + originOffset.x
				                + pixelX(textStyle.getMarginLeft()), part.origin.y + originOffset.y + (i * lineHeight),
				                true);
