...
class PBox
{
	// Styles
	/**
	 * Style flag that forces the PBox to be in a new line below the previous one Cannot be used simultaniously to
	 * POS_RIGHT.
	 */
	public static final int POS_BELOW = 1;

	/**
	 * Style flag that forces the PBox to be in line with the previous one. Cannot be used simultaniously to POS_BELOW.
	 */
	public static final int POS_RIGHT = 2;

	/**
	 * Style flag that can be used additionally to POS_BELOW/POS_RIGHT to tell the PBox that it shoul consume all
	 * available horizontal space on the page.
	 */
	public static final int GRAB = 4;

	/**
	 * Style flag that is mainly used by tables and forces all PBoxes in a line to have the same height. All PBoxes in
	 * the line should be constructed with this flag
	 */
	 ...
	 * 
	 * @param parent
	 * @param style Poition: POS_BELOW or POS_RIGHT.
	 */
	 ...
	 * 
	 * @param parent
	 * @param style Poition: POS_BELOW or POS_RIGHT. Also GRAB and/or ROW_ALIGN.
	 * @param hWeight Determines, how much of the existing page width should be occupied. 1.0 = full page. -1 = fiexed
	 *            width, see minCm.
	 * @param minCm Minimum width in cm. If > 0.0, the box will at least have this width.
	 */
	...
	/*
	 * Sets the forced height. This means that the PBox will have this height (in pixel) regardless of how high it wants
	 * to be.
	 */
	...
	public void draw(int page, Point originOffset)
	{
		...
		if(layoutIsOnPage(page))
		{
			...
			{
				gc.fillRectangle(originForDrawing.x + width - boxStyle.getLineWidth(1), originForDrawing.y, boxStyle
				                .getLineWidth(1), height);
			}
			if(boxStyle.hasLine(2))
			{
				gc.fillRectangle(originForDrawing.x, originForDrawing.y + height - boxStyle.getLineWidth(2), width,
				                boxStyle.getLineWidth(2));
			}
	...
	/*
	 * Some elements can occupy more than one Page. Therefore this function tests if the element has a part on the given
	 * page. @param page @return boolean
	 */
	...
	/*
	 * Returns the space in y-direction the Element would occupy of the rest of the page if told so. Convention: this
	 * method can be called several times for one page, but only until layoutOccupy is called once for this page.
	 * @param spaceLeft @return int -1, if the element deciedes not to have any part on the given page
	 */
	...
	/*
	 * Returns true if the box would fit or at least finish into/within the given space in y-direction. Convention: this
	 * method can be called several times for one page, but only until layoutOccupy is called once for this page. @param
	 * spaceLeft
	 */
	...
	/*
	 * Tells the element to occupy the given space on the page. Returns the space in y-direction the Element occupys of
	 * the rest of the page. Convention: this method is only called once for one page, and after this call there will be
	 * no further layoutHowMuchWouldYouOccpy-calls for this page. @param spaceLeft @return int
	 */
	...
	/*
	 * Gives the vertical size of the element. Used by all layout* functions. If multipage functionallity is needed,
	 * this mechanism does no longer work. Use/overwrite getHeight(int page) instead.
	 */
