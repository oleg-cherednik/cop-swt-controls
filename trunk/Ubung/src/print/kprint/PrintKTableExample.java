...
class PTableBoxProvider
{
	...
	public PBox createBox(PContainer parent, int style, int col, int row, int widthFromModel, int heightFromModel,
	                boolean fixed, Object content)
	{
		// create a text box
	...
	protected void fillDocument()
	{
		...
		for(int i = 0; i < model.getRowCount(); i++)
		{
			...
			for(int j = 0; j < model.getColumnCount(); j++)
			{
				...
				PBox box = boxProvider.createBox(parent, style, j, i, model.getColumnWidth(j), height, (model
				                .getFixedColumnCount() > j || model.getFixedRowCount() > i), model.getContentAt(j, i));
...
class PPageNumber extends PTextBox
{
	...
	public void draw(int page, Point originOffset)
	{
		...
		if(layoutIsOnPage(page))
		{
			...
			gc.drawText("" + pageNumber, origin.x + originOffset.x + pixelX(textStyle.getMarginLeft()), origin.y
			                + originOffset.y, true);
		}
	}
	...
	public void draw(int page, Point originOffset)
	{
		...
		if(layoutIsOnPage(page))
		{
			...
			for(int i = 0; i < textLines.size(); i++)
			{
				...
				gc.drawText((String)textLines.get(i), origin.x + alignPixel + originOffset.x
				                + pixelX(textStyle.getMarginLeft()), origin.y + originOffset.y
...
class PImageBox extends PBox
{
	...
	public void draw(int page, Point originOffset)
	{
		if(layoutIsOnPage(page))
		{
			...
			if(image != null)
			{
				gc.drawImage(image, 0, 0, imgOriginalSize.x, imgOriginalSize.y, origin.x + originOffset.x, origin.y
				                + originOffset.y, imgTargetSize.x, imgTargetSize.y);
			}
		}
	}
...
class PDocument extends PContainer
{
	 ...
	 */
	public PDocument(double pageWidth, double pageHeight, double top, double right, double bottom, double left,
	                double scale, String docTitle)
	{
		super(null);
		this.doc = this;
		this.scale = scale;
	...
	{
		Point erg = new Point(PBox.pixelX(pageWidth - margins[1] - margins[3]), PBox.pixelY(pageHeight - margins[0]
		                - margins[2]));
	...
	public void layout()
	{
		...
		originOnFirstPage = new Point(PBox.pixelX(margins[3]) + offsetX, PBox.pixelY(margins[0])
		                + getFirstHeader().getHeight() + offsetY);
		// System.out.println(" originOnFirstPage: "+originOnFirstPage);

		originOnOtherPages = new Point(PBox.pixelX(margins[3]) + offsetX, PBox.pixelY(margins[0])
...
class MsgBox
{
	...
	{
		s.pack();
		s.setLocation((d.getBounds().width - s.getBounds().width) / 2,
		                (d.getBounds().height - s.getBounds().height) / 2);
...
class KPrintExample
{
	...
	public KPrintExample()
	{
		...
		generateParagraph(doc, "KPrint is a collection of Java classes that allows "
		...
		generateParagraph(doc, "In SWT, you use Layouts "
		                + "to determine how the widgets are arranged on the window. KPrint "
		                + "has just one simple layout concept.");
		generateParagraph(doc, "The elements you can place on the paper are text "
		...
		generateImage(doc, "/gfx/fig1.gif", "Figure 1: The position of an element is "
		...
		generateParagraph(doc, "This layout concept is both simple but powerful. "
		...
		generateParagraph(doc, "This text is created using the KPrint framework. "
		...
		generateParagraph(doc, "Printing a Table works like that: you just add "
		...
		generateParagraph(doc, "KPrint offers also the possibility to print "
...
{
	private final static String[][] content = { { "Name", "Kupzog", "Hansson", "Walter", "Hutton" },
...
class KTable extends Canvas
{
	...
	protected void drawBottomSpace(GC gc)
	{
		...
		{
			r.y = m_Model.getFirstRowHeight() + (m_Model.getFixedRowCount() - 1 + m_RowsVisible)
	...
	protected void drawCell(GC gc, int col, int row)
	{
		...
		m_Model.getCellRenderer(col, row).drawCell(gc, rect, col, row, m_Model.getContentAt(col, row),
	...
	public boolean isCellVisible(int col, int row)
	{
		if(m_Model == null)
			return false;
		return ((col >= m_LeftColumn && col < m_LeftColumn + m_ColumnsVisible && row >= m_TopRow && row < m_TopRow
		                + m_RowsVisible)
	...
	public boolean isCellFullyVisible(int col, int row)
	{
		if(m_Model == null)
			return false;
		return ((col >= m_LeftColumn && col < m_LeftColumn + m_ColumnsFullyVisible && row >= m_TopRow && row < m_TopRow
		                + m_RowsFullyVisible)
	...
	public int resizeColumnOptimal(int column)
	{
		if(column >= 0 && column < m_Model.getColumnCount())
		{
			...
			{
				int width = m_Model.getCellRenderer(column, i).getOptimalWidth(m_GC, column, i,
	...
	{
		if(col < m_Model.getColumnCount() && col >= m_Model.getFixedColumnCount() && row < m_Model.getRowCount()
...
class KTableCellRenderer
{
	                ...
	                boolean clicked)
	{
		if(fixed)
		{
			...
			if(clicked)
			{
				SWTX.drawButtonDown(gc, content.toString(), SWTX.ALIGN_HORIZONTAL_LEFT | SWTX.ALIGN_VERTICAL_CENTER,
				                null, SWTX.ALIGN_HORIZONTAL_RIGHT | SWTX.ALIGN_VERTICAL_CENTER, rect);
			}
			else
			{
				SWTX.drawButtonUp(gc, content.toString(), SWTX.ALIGN_HORIZONTAL_LEFT | SWTX.ALIGN_VERTICAL_CENTER,
				                null, SWTX.ALIGN_HORIZONTAL_RIGHT | SWTX.ALIGN_VERTICAL_CENTER, rect);
			}
		...
		SWTX.drawTextImage(gc, content.toString(), SWTX.ALIGN_HORIZONTAL_CENTER | SWTX.ALIGN_VERTICAL_CENTER, null,
		                SWTX.ALIGN_HORIZONTAL_CENTER | SWTX.ALIGN_VERTICAL_CENTER, rect.x + 3, rect.y, rect.width - 3,
		                rect.height);
