...
class SWTPTable
{
	...
	protected void fillDocument()
	{
		...
		// Zeilen
		/**
		 * TODO Print Table Header if (j == 0) style = PBox.POS_BELOW | PBox.ROW_ALIGN;
		 */
		...
		for(int j = 0; j < table.getColumnCount(); j++)
		{
			...
			PBox box =
			                boxProvider.createBox(parent, style, j, 0, table.getColumn(j).getWidth(), height, true,
		...
		for(int i = 0; i < table.getItemCount(); i++)
		{
			...
			for(int j = 0; j < table.getColumnCount(); j++)
			{
				...
				if(j == 0)
					style = PBox.POS_BELOW | PBox.ROW_ALIGN;
				PBox box =
				                boxProvider.createBox(parent, style, j, i, table.getColumn(j).getWidth(), height,
				                                false, table.getItem(i).getText(j));
				double boxWidth = Math.max(box.minCm, parent.getPossibleWidth() * box.hWeight);
				width -= boxWidth;
				if(width < 0)
				{
